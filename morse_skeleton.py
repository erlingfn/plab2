...
class morse_decoder():
    ...
    def decoding_loop(self):
        while True:
            ...
            # serial port returns three bytes, so original code does not work
            # for byte in s:
    ...
    def handle_symbol_end(self):
        if(not morse_decoder._morse_codes.keys().__contains__(self.current_symbol)):
            # reset current symbol when user fails at morse code
            # or beginning of message
            self.current_symbol = ''
            return
        temp_symbol = morse_decoder._morse_codes.get(self.current_symbol)
        self.update_current_word(temp_symbol)
        # reset current_symbol
        self.current_symbol = ''

    def handle_word_end(self):
        self.handle_symbol_end()
        # Do not start message with space
